...
export function createNeoformRig(compact: boolean) {
  const boneGeometry = new THREE.CylinderGeometry(0.055, 0.045, 1, compact ? 5 : 6, 1);
  const jointGeometry = new THREE.IcosahedronGeometry(0.075, 0);
  ...
  const group = new THREE.Group();
  group.add(past.group, future.group, current.group, coreHalo, core);
  ...
  const updateLayer = (layer: RigLayer, pose: Float32Array, signalProgress: ArrayLike<number>) => {
    for (let boneIndex = 0; boneIndex < BONE_PAIRS.length; boneIndex += 1) {
      ...
      for (let signalIndex = 0; signalIndex < signalProgress.length; signalIndex += 1) {
        ...
        const head = (progress - 0.16) * 1.18;
        signal = Math.max(signal, 1 - smoothstep(0.02, 0.16, Math.abs(route - head)));
      }
      coolSpectralColor(performance.now() * 0.001, boneIndex * 0.11, color);
    ...
    for (let jointIndex = 0; jointIndex < NEOFORM_JOINT_COUNT; jointIndex += 1) {
      ...
      matrix.compose(midpoint, new THREE.Quaternion(), scale.setScalar(jointScale));
  ...
  const update = (input: NeoformRigUpdate) => {
    ...
    updateLayer(current, currentPose, input.signalProgress);
    updateLayer(past, pastPose, input.signalProgress);
    updateLayer(future, futurePose, input.signalProgress);
    past.group.visible = input.predictionStrength > 0.02 && !input.reducedMotion;
    future.group.visible = input.predictionStrength > 0.02 && !input.reducedMotion;
