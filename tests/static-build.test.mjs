...
test("builds a complete GitHub Pages artifact", async () => {
  ...
  assert.match(rig, /TetrahedronGeometry/);
  assert.match(rig, /PLATE_JOINTS/);
  assert.match(rig, /assembly/);
  ...
  assert.match(soundtrackEngine, /playFootstep/);
  ...
  assert.doesNotMatch(shader, /aTarget1|uMorphBias|morphPosition/);
  assert.match(world, /uSignalProgress\[5\]/);
  assert.match(world, /signalIndex < 5/);
  assert.match(world, /aGroundPosition/);
  assert.match(world, /aSwarmTarget/);
  assert.match(world, /pointerField/);
  assert.match(world, /footField/);
  assert.match(rig, /head = \(progress - 0\.06\) \* travel/);
  assert.match(rig, /signalTravelRate/);
  assert.match(rig, /tipGlows/);
  ...
  assert.match(world, /uReducedMotion/);
