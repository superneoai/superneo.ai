...
test("builds a complete GitHub Pages artifact", async () => {
  ...
  assert.match(app, /const DISCOVERY_REWARD_MS = 4_000;/);
  assert.match(app, /const FOOTER_NOTICE = "SUPERNEO™ © 2026 ACTUAL LTD\.";/);
  assert.match(app, /className="discovery-copy" data-found=\{discoveryRewardVisible\}/);
  assert.match(app, /className="discovery-message discovery-message--notice"/);
  assert.match(app, /className="discovery-message discovery-message--reward"/);
  ...
  assert.doesNotMatch(app, /Your idea starts here|Agents work in your engine|You own the result|Signals converge|between them/);
  ...
  assert.match(app, /const progressStages = \["01 LATENT", "02 EMERGE", "03 NEO"\]/);
  assert.match(app, /const stageTotal = String\(STAGE_COUNT\)\.padStart\(2, "0"\)/);
  ...
  assert.doesNotMatch(morph, /aTarget3/);
  ...
  assert.doesNotMatch(shader, /sparkPulse|swarmWave|resultRotation|routeWave|inferenceState|aTarget3/);
  ...
  assert.match(shader, /latentGlyph/);
  assert.match(shader, /emergenceGlyph/);
  assert.match(shader, /openGlyph/);
  assert.doesNotMatch(shader, /sparkGlyph|swarmGlyph|resultGlyph|inferenceGlyph/);
  ...
  assert.match(sitemap, /<loc>https:\/\/superneo\.ai\/legal\/<\/loc>/);
  ...
  await access(new URL("../dist/legal/index.html", import.meta.url));
});
